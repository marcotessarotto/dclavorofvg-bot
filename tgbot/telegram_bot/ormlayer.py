...
def orm_get_current_user_context(telegram_user_id: int) -> CurrentUserContext:
    if not (
        obj := orm_get_obj_from_cache(
            f"_current_context_users_{telegram_user_id}"
        )
    ):
    ...
    return None if obj.timestamp < d else obj
...
def orm_get_vacancies_published_today(refresh=False, create_news_item=False) -> str:
    ...
    for counter, d in enumerate(vacancies, start=1):
