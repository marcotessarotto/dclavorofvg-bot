...
class CustomDateTimeField(models.DateTimeField):
    def value_to_string(self, obj):
        if val := self.value_from_object(obj):
...
class MessageToUser(models.Model): # message to specific user from admins
    ...
    def __str__(self):
        return f'MessageToUser {str(self.telegram_user.user_id)} : {self.text}'
...
class UserFreeText(models.Model):  #
    ...
    def __str__(self):
        return f'UserFreeText {self.telegram_user.user_id} : {self.text}'
...
class NewsFile(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.id} : {self.file_field.name}"
...
def news_item_signal(sender, instance: NewsItem, *args, **kwargs):
    ...
    latest_timestamp = max(latest_timestamp, now_aware)
